...
This directory governs golden-set versioning and a multi-model recall/MRR/nDCG harness for
Scrutator's hybrid search endpoint (`/v1/search`), with a runtime liveness pre-flight so a
...
- **`benchmark/recall-gate/`** — the standing LTM recall CI gate that runs
...
- **`benchmark/ml-vs-llm/`** — the ML-vs-LLM extraction-task benchmark suite:
  ...
  `/v1/search`) and does not touch NER/RE/classification. Extend `ml-vs-llm/` for those tasks,
  reusing this directory's golden-set governance pattern, not its code.
- **Former host-local scripts** — ad hoc benchmark scripts that ran once on a production host
  and were never committed could not be recovered. This directory exists specifically so that
  failure mode stops recurring: everything here is git-tracked.
...
| `measure.py` | The original one-off script, preserved as the pre-`harness.py` reference implementation. It remains frozen and is not extended further. |
...
| `golden-arcanada-v0.results-summary.json` | The baseline measurement (`measure.py` output) — overall recall@5 = 0.909. `harness.py` must reproduce this within ±0.03 before it is trusted as `measure.py`'s replacement. |
| Historical recall report | The original baseline report and golden-set methodology writeup, preserved verbatim. |
...
- `golden-arcanada-v0.jsonl` — the original 33-row seed.
...
python benchmark/scrutator/harness.py \
    ...
    --models bge-m3 \
    --namespace arcanada \
    --bearer-token-file /private/path/scrutator-reader.jwt
```

The token file is mandatory for live calls, must have mode `0600`, and must contain a
short-lived reader token authorized for the requested namespace. The GitHub Actions workflow
mints this token from the existing `KB_OBSERVER_CLIENT_SECRET` and removes it in an `always()`
cleanup step. Never pass a token as a command-line value.
...
## Running the paired rerank gate
