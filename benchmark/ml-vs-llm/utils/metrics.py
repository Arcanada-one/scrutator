...
def compute_classification_metrics(predicted: list[str], golden: list[str]) -> dict:
    ...
    correct = sum(1 for p, g in zip(predicted, golden, strict=False) if p == g)
    ...
    for label in all_labels:
        tp = sum(1 for p, g in zip(predicted, golden, strict=False) if p == label and g == label)
        fp = sum(1 for p, g in zip(predicted, golden, strict=False) if p == label and g != label)
        fn = sum(1 for p, g in zip(predicted, golden, strict=False) if p != label and g == label)
    ...
    return {
        ...
        "per_class": per_class,
        "support": len(golden),
        "confusion": dict(Counter(zip(golden, predicted, strict=False))),
    }
